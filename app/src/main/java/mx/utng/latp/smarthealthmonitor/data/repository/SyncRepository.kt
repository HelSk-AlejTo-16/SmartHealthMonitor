package mx.utng.latp.smarthealthmonitor.data.repository
...
import mx.utng.latp.smarthealthmonitor.data.remote.NeonClient
import mx.utng.latp.smarthealthmonitor.data.remote.NeonRequest
        ...
        withContext(Dispatchers.IO) {
            NeonClient.api.executeQuery(
                ...
                request = NeonRequest(
                    ...
                    params = listOf(lectura.bpm.toString(), lectura.estado, lectura.dispositivo, lectura.hora) // Convert bpm to string to match NeonRequest param list type
                )
            )
        }
    ...
    suspend fun sincronizarDesdeNeon(limite: Int = 50) = withContext(Dispatchers.IO) {
        val response = NeonClient.api.executeQuery(
            ...
            request = NeonRequest(
                ...
                params = listOf(limite.toString()) // Convert to string
            )
        )
